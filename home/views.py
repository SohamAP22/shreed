...
def edit(request):
    if request.method == "POST":
        ...
        if edit_weight != '':
            ...
            edit_amount = int(edit_weight)*edit_reel_entry.reel_rate
            edit_reel_entry.reel_balance= edit_weight
            edit_reel_tran.reel_old_balance=edit_reel_tran.reel_new_balance
            edit_reel_tran.reel_new_balance=edit_weight

        if  edit_rate != '':
            edit_amount = edit_reel_entry.reel_weight*int(edit_rate)
...
def report(request):
    gsms=reel_entry.objects.values_list('reel_gsm', flat=True).distinct()
    #gsms_count= gsms.count()
    #print(gsms_count)
    #print(gsms[2])
    gsm = {'gsms':gsms}
    print(gsms)
    return render(request,'report.html',gsm)
...
def report_gsm(request):
    if request.method == "POST":
        ...
        use_size = request.POST.get('req_size')
        show=reel_entry.objects.filter(reel_gsm=use_gsm, reel_size=use_size).all()
